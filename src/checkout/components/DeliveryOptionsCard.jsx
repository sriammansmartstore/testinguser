...
const DeliveryOptionsCard = ({ orderTotal = 0 }) => {
  ...
  return (
    <Card sx={{ borderRadius: { xs: 0, sm: 2 }, boxShadow: { xs: 'none', sm: 2 }, mt: 1 }}>
      <CardContent>
        <Typography variant="h6" sx={{ fontWeight: 800, mb: 1 }}>Delivery Options</Typography>
            ...
            <Box sx={{ flex: 1 }}>
              {nextTier === 'standard' && (
                <Typography variant="body2" sx={{ fontWeight: 800 }}>
                  Add ₹{remainingForStandard} more to get FREE Standard Delivery
                </Typography>
              )}
              {nextTier === 'express' && (
                <Typography variant="body2" sx={{ fontWeight: 800 }}>
                  You're close — add ₹{remainingForExpress} more to unlock FREE Express Delivery
                </Typography>
              )}
              {!nextTier && (
                <Typography variant="body2" sx={{ fontWeight: 800, color: 'success.main' }}>
                  Congratulations — you have FREE Express Delivery!
                </Typography>
              )}
            </Box>
            <Box sx={{ display: { xs: 'none', sm: 'block' } }}>
              <Stack spacing={0.5} alignItems="flex-end">
                <Box sx={{ display: 'flex', gap: 1 }}>
                  <Chip label={`Free standard over ₹${standardThreshold}`} size="small" sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider' }} />
                  <Chip label={`Free express over ₹${expressThreshold}`} size="small" color="warning" variant="outlined" />
                </Box>
              </Stack>
            </Box>
          </Box>
        ...
        <RadioGroup value={option} onChange={handleChange}>
          <FormControlLabel
            ...
            label={
              <Box sx={{ textAlign: 'left' }}>
                <Typography variant="body1" sx={{ fontWeight: 800 }}>Standard</Typography>
                <Typography variant="caption" color="text.secondary">Delivered in 24 hrs</Typography>
                <Box sx={{ mt: 0.5 }}>
                  <Chip size="small" label="₹49" sx={{ mr: 0.5 }} />
                  <Chip size="small" color="success" variant="outlined" label={`Free above ₹${standardThreshold}`} />
                </Box>
              </Box>
            }
          />
          <FormControlLabel
            ...
            label={
              <Box sx={{ textAlign: 'left' }}>
                <Typography variant="body1" sx={{ fontWeight: 800 }}>Express</Typography>
                <Typography variant="caption" color="text.secondary">Delivered within 1 hrs</Typography>
                <Box sx={{ mt: 0.5 }}>
                  <Chip size="small" label="₹99" sx={{ mr: 0.5 }} />
                  <Chip size="small" color="success" variant="outlined" label={`Free above ₹${expressThreshold}`} />
                </Box>
              </Box>
            }
          />
