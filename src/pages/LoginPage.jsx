  ...
  return (
    <Box className="login-root">
      <Box className="login-box" sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
        <Typography variant="h5" className="login-title">{t('welcomeBack', 'Welcome Back')}</Typography>
        {(error || info) && (
        ...
        {otpSent && (
          <TextField label={t('enterOtp', 'Enter OTP')} placeholder={t('sixDigitCode', '6-digit code')} variant="outlined" fullWidth margin="normal" value={otp} onChange={e => setOtp(e.target.value)} inputProps={{ maxLength: 6 }} />
        )}
        {!otpSent ? (
          ...
          <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            <Button variant="contained" className="login-btn" fullWidth onClick={verifyOtpAndLogin} disabled={verifying || !otp}>{verifying ? <CircularProgress size={20} /> : t('verifyLogin', 'Verify & Login')}</Button>
        ...
        >
          ...
          <Box sx={{ textTransform: 'none', fontWeight: 700 }}>
            {googleLoading ? (t('sendingOtp', 'Signing in...')) : t('loginWithGoogle', 'Login with Google')}
          </Box>
        </Button>
        <Button
          variant="text"
        ...
        <Typography className="switch-link" onClick={() => navigate("/signup")}>{t('dontHaveAccount', "Don't have an account? Sign Up")}</Typography>
      </Box>
    </Box>
  );
};
